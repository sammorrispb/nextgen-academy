...
- **Body:** `{ parentEmail | parentPhone, summary, messageId?, messageSource?, channel?, observedAt?, location?, landingPage?, dryRun? }`. `location` is allowlisted against `LEAD_LOCATIONS` so an email parse can't invent a Notion select option; `channel` against `ENRICH_CHANNELS` (`Email` default, `iMessage`, `Text`, `WhatsApp`, `Meeting`, `Note`, `Call`, `Form`) and `messageSource` against `gmail` (default) / `imessage` / `open_brain` for the same reason — the vocabulary inside Notes is closed too.
- **One engine, three callers.** The route calls the SAME `appendLeadInquiry` the form path uses, so an Open-Brain-derived update and a form-derived update cannot drift. **The live caller is Open Brain (2026-09-03):** its `crm` agent's `writeback_nga_crm` job (open-brain `supabase/functions/nga-crm-sync/writeback.ts`, 09:15 + 14:15 UTC) pushes every NGA `lead_activities` row that did NOT come through this site — a parent's email (the "NGA email to CRM enrichment" routine files those into Open Brain, not Notion), an iMessage, a meeting note, a coach's `update_pipeline_stage` note — one call each. Website-originated activities (`channel = nga` / `nga_*`) are excluded there because the site already wrote them. Only `LEAD_ENRICH_SECRET` (Vercel) + the same value as `NGA_LEAD_ENRICH_SECRET` (Supabase) stand between the two; unset on either side, the job fails loudly rather than no-opping green.
- **Idempotent on `messageId`, across callers** — the appended line carries a marker built from the message's OWN id where it has one: `[gm:<gmail id>]`, `[im:<iMessage GUID>]`, or `[ob:<Open Brain activity id>]` when there is no message id. A marker already present is a no-op, so a re-run of any caller is safe, and a mail scan and Open Brain pushing the same email produce ONE line. Ids from different systems never collide on one token (prefixes differ). No new Notion property needed. `messageId` must be one opaque token (`MESSAGE_ID_RE`) so it can never break the guard.
- **`Last Contact Date` only moves forward.** Open Brain pushes oldest-first and retries stragglers days later; an older line still lands in Notes but never drags the row's last-heard-from date back behind a newer one. The response carries `pageId` so a caller's own ledger (`crm_writebacks` in Open Brain) can record where the line went.
- **Family lookup tolerates the real data.** `findFamilyRows` tries Notion's exact `equals` on Parent Email first, then a `contains` + case-insensitive-equality re-check (a parent who typed `Jane@Gmail.com` on the form arrives lowercased from Open Brain); phone-only lookups net on the last four digits and settle on the normalized ten, because the CRM stores `(240) 555-0134` beside `+12405550134`. Neither widens past the one family — pinned in the egress spec.
...
- **The scheduling lives in Open Brain, not in a Vercel cron.** A cron here would need Google OAuth credentials plus a refresh-token store in production — the tradeoff `docs/admin-reduction-roadmap.md` already declined for the calendar sync. Open Brain already holds the structured activity (`lead_activities`, keyed on the Gmail/iMessage id) and a job queue with retries and observability, so the push is a deterministic worker there rather than an LLM re-reading the inbox; it runs whether or not Sam's Mac is awake. A cloud routine may still call this endpoint directly — the marker rule keeps the two from stacking.
...
- **`OpenNowOffers` (`src/lib/open-now-offers.ts`, pure + date-injected)** renders what
  ...
  fall on `NEXT_PUBLIC_FALL_REGISTRATION_OPEN` + the season's last Sunday, Pickl Park
  on its registration window (`picklParkRegistrationOpen()` — open by default through
  its last Saturday, flag as kill switch), league on its `registrationDeadline`. **No seat counts** (they need a Notion read; a fabricated
...
- **$225 matches Montgomery County and buys a shorter block** (6 × 60 min vs Walter Johnson's 6 × 90). The venue closes that gap, not the clock: indoors means all six Saturdays run, while the outdoor season holds two rain dates because it might not. `PICKLPARK_INDOOR_NOTE` is reused verbatim by the page and the confirmation email — **a surface that quotes the price without it is selling the shorter hour and none of the reason.** There is no held date at all now (2026-09-20): Oct 31 is a playing Saturday, and indoors nothing takes a week away. `PICKLPARK_MAKEUP_DATES` is an EMPTY `readonly string[]` — a supported state, and the three surfaces that render it (the page, the success page, the season confirmation email) print nothing rather than a dangling "we make it up on .". Adding a hold back is one entry in that array.
...
- **No age-floor exception any more (retired 2026-09-03).** NGA's 6–16 rule is unchanged and every NGA form still starts at 6. The Wednesday club used to carry `ageMin: 5` because Rosemary Hills is a PreK–2 building; with the move to DuFief (K–5) that justification is gone and the club reverts to `ageMin: 7` like the other four — so **no club in this program now sits below the NGA floor.** If a partner asks for a lower band again, scope it to this file as a per-club `ageMin` exactly as `MVF_AGE_MIN = 8` narrows in its own, and never propagate it to an NGA surface.
- **Schedule is CONFIRMED (Stef's revised "Coach Sam — Weekly Schedule" PDF, 2026-09-09, superseding the 2026-08-13 revision).** All five clubs run mid-Sept → mid/late Nov with real session dates already reconciled against the MCPS 2026–27 calendar — the gaps in each club's date list are closures, so a "missing" week is not an error. **Times: Mon–Thu 3:20–4:30 PM, Fri 3:50–5:00 PM** — 70-minute blocks the partner publishes; copy them, never derive them from dismissal. No EC calendar block ships all-day. **Venue history (keys never move):** *2026-08-16* — Friday moved Sherwood ES (Sandy Spring) → Olney ES; *2026-09-03* — Wednesday moved Rosemary Hills ES (Silver Spring) → DuFief ES (North Potomac); *2026-09-09* — the revised PDF moved Friday BACK to Sherwood ES. Every move kept its session dates and its original key (`silver-spring-wed`, `sandy-spring-fri`) so calendar blocks update in place instead of churning — read `town` from the file, never from the key.
...
- **MVF venues are per-program.** `MvfProgram.venue` is on each row because MVF moves the sessions: the Aug 27 intro is at Apple Ridge, and both fall sessions are at North Creek. There is no single MVF venue constant. **Fall I moved Watkins Mill → North Creek on 2026-08-27** (Marnovan Alvero, MVF) — that note supersedes the Rec Guide, and `WATKINS_MILL` is now only the contingency if the North Creek renovation starts mid-session (kept exported on purpose; don't prune it). The move took 7 days to reach this repo while the calendar was hand-patched, so `/calendar-sync` was trying to "fix" a correct calendar back to the wrong court — **when a partner moves a venue, `src/data/mvf.ts` is the fix**; a calendar edit alone rots.
...
Pricing is **one flat rate per 1-hour slot, drop-in only — no subscription, no refunds**. The rate is `STRIPE_DROPIN_PRICE_ID` (still $20) and is **never printed on a parent-facing surface** as of 2026-09-08 — the parent first sees it on the Stripe checkout page, so any new page, feed or email must describe the format ("one hour, drop-in") and let checkout state the number. Pinned by `e2e/invariant-dropin-price-not-quoted.spec.ts`. **No exceptions, including the card-on-file commit flow** (`/commit/[token]`, `CommitForm`, the commit success page, `commit-confirmation.ts`) — Sam's explicit call, 2026-09-08. That flow saves a card through a Stripe `mode: "setup"` session, which shows the parent no amount, and `crew-autoreserve` then charges off-session, so the *copy* carries the disclosure the figure used to: each of those four surfaces ties the charge to **the rate the parent already paid for the first session** and to the weeks it applies to. That anchor is load-bearing and is pinned too — "we'll charge your card" alone is not the same promise, and editing it down to that is the regression the spec exists to catch. Sessions split into Early and Late slots — pick one or both (two slots = two drop-in charges until the planned two-hour bundle ships). Each session opens for registration **30 days ahead** and caps at 4 players per pickleball court.
...
- **Schema.org JSON-LD** is required: ONE organization node in the root layout (`organizationJsonLd()` — `@type` SportsOrganization + SportsActivityLocation, `@id` `https://nextgenpbacademy.com/#organization`), `FAQPage` + `Person` (per coach, with `@id`) on the home page, one `SportsEvent` per upcoming session on `/schedule`. Every organizer / provider / publisher / parentOrganization / worksFor uses `orgRef()` — never an inline org object (`e2e/entity-graph.spec.ts`). Use the `<JsonLd />` component.
...
- `STRIPE_PICKLPARK_SEASON_PRICE_ID` + `NOTION_PICKLPARK_REGS_DB_ID` — the Pickl Park
  Saturday season pair, both set in production since 2026-08-31. The price env is the
  charge guard (`/api/checkout-picklpark` 503s without it). Never point
  `NOTION_PICKLPARK_REGS_DB_ID` at the Fall Regs DB — capacity is scoped by
  `Group` alone, so the two seasons would cross-count seats.
- `NEXT_PUBLIC_PICKLPARK_REGISTRATION_OPEN` — **a kill switch since 2026-09-05, not a
  launch flag** (the opposite of `NEXT_PUBLIC_FALL_REGISTRATION_OPEN`). Unset or `true`
  = the `/picklpark` form renders through the season's last Saturday; any other value
...
- `LEAD_ENRICH_SECRET` — Bearer secret for the off-website → CRM enrichment surface (`POST /api/lead-enrich`). Lets Open Brain's `writeback_nga_crm` job (and any mail-scan routine) append what a parent told us over email / iMessage / in a meeting onto their Player CRM row, through the same writer the lead form uses. **Must equal `NGA_LEAD_ENRICH_SECRET` in the open-brain Supabase secrets** — the job 401s and aborts loudly otherwise. Distinct from `NGA_ADMIN_SECRET` to isolate blast radius. Unset = the route fails closed (401) and the surface ships dark.
