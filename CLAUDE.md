...
- **`GET /api/cron/seed-tuesday-sessions`** — schedule `0 8 * * 1` UTC (Mon ~3–4am ET). Keeps the recurring **all-levels Tuesday** ("Olney Tuesday Evening") stocked: ensures each of the next `WEEKS_AHEAD` (8) Tuesdays has one row per level (Red/Orange/Green/Yellow) — a court each — at the hidden-location Olney 6–7 PM slot. **Idempotent** (any existing row for a date+level is left alone, whatever its Status — so a deliberately-cancelled Tuesday/level is never resurrected) and **fail-soft per row**. Logic in `src/lib/recurring-sessions.ts` (`ensureAllLevelsTuesdays`); pure helpers (`upcomingTuesdays`, `buildTuesdayRowProps`) unit-tested in `e2e/recurring-sessions.spec.ts`. Rows are hidden-location — Sam still fills the exact Location before the reveal cron fires. Edit the venue/time/levels via the `TEMPLATE`/`TUESDAY_LEVELS` constants in the lib.
