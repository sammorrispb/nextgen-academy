...
4. **Thu 6pm ET — cron picks up.** `fetchApprovedNewsletterDraft()` in `src/lib/notion-newsletter-drafts.ts` queries the DB for the most recent `Status = Approved` row with `Drafted At` within the last 7 days, fetches the row's child blocks via the Notion REST API, and converts them to email-safe HTML (heading_2/3, paragraph, divider, bulleted_list_item, numbered_list_item — with bold/italic/code/link rich-text). The HTML lands in the `newsletterLeadHtml` field of `WeeklyNewsletterInput`; the template renders it inside an accent-lime card under the "From Coach Sam this week" eyebrow. Fails soft on any Notion error so the cron still ships the rest of the email.

The cron NEVER writes back to the drafts DB. The 7-day window is the only freshness guard — a stale Approved row from earlier than 7 days ago is silently ignored on next Thursday's send. The drafter writing a fresh Pending row every Wednesday means the latest week's Approved row always wins on tie.
...
- **`GET /api/cron/seed-tuesday-sessions`** — schedule `0 8 * * 1` UTC (Mon ~3–4am ET). Keeps the recurring **all-levels Tuesday** ("Redland Tuesday Evening") stocked: ensures each of the next `WEEKS_AHEAD` (8) Tuesdays has one row per level (Red/Orange/Green/Yellow) — a court each — at the Redland 6–7 PM slot. **Idempotent** (any existing row for a date+level is left alone, whatever its Status — so a deliberately-cancelled Tuesday/level is never resurrected) and **fail-soft per row**. Logic in `src/lib/recurring-sessions.ts` (`ensureAllLevelsTuesdays`); pure helpers (`upcomingTuesdays`, `buildTuesdayRowProps`) unit-tested in `e2e/recurring-sessions.spec.ts`. Rows seed with the exact `Location` (Redland Middle School tennis courts, 6505 Muncaster Mill Rd, Rockville, MD 20855 — set 2026-06-09; previously Rosa Parks MS) plus a broad `Public Area` ("Derwood, MD") fallback — session locations are public (the hidden-location/reveal-cron system was retired 2026-06-05). Edit the venue/time/levels via the `TEMPLATE`/`TUESDAY_LEVELS` constants in the lib.
