...
The CONFIRMED season announcement + one-click poll to **active families only** — distinct from the `/fall` demand-sizing survey above. Sam set the terms 2026-08-14 (`src/data/fall-poll-2026.ts`): **6 Sundays Sept 20 – Oct 25 at Walter Johnson HS (moved from Wood MS 2026-08-27), Green 1:00–2:30 PM / Yellow 2:30–4:00 PM, 8 spots each, $225 per player** — so this template DOES quote the price (a real operator-set number; the no-quoting rule targets prices that don't exist yet).
