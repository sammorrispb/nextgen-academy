...
- **Dates are CONFIRMED (2026-08-09, Stef's schedule PDF).** All five clubs run mid-Sept → mid/late Nov with real session dates already reconciled against the MCPS 2026–27 calendar — the gaps in each club's date list are closures, so a "missing" week is not an error. The PDF also swapped the Derwood and Silver Spring clubs' weekdays vs. the July hold (times follow the school, Sam's call 2026-08-09) and added the fifth Friday club. Belmont's and Sherwood's times are still unannounced → `startTime: null` → all-day blocks; add the time when Stef announces it and the sync converts the blocks in place.
