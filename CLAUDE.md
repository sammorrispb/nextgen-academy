...
Demand-sizing for a **Fall 2026 season at Earle B. Wood Middle School, Saturdays + Sundays 5–7 PM, 8 weeks (Sep 12 – Nov 1, 2026)** — two programs in one window: the **NGA youth season** (2 hrs = 1 hr coached practice + 1 hr rotating-partner round robin, by color group) and the **Link & Dink adult round robin** (2 hrs rotating-partner play, no practice hour, by L&D bracket; parents encouraged to join). **9 slots per group, first come first serve, full-season commitment paid up front, with a sub list for week-to-week.**

- **Season config is `src/data/fall-2026.ts`** — venue, the 8 Saturday + 8 Sunday ISO dates (written out, never computed: date arithmetic on a UTC build server is the documented footgun), both program definitions, `SLOTS_PER_GROUP`, and the price bands. The page, the form, the confirmation email, and the broadcast all render from it; editing the season is a one-file change.
- **NOTHING IS BOOKABLE.** There is no Stripe product for a season and the court permit covers far fewer players than 9 × the group count. Per the standing rule, **no price is quoted anywhere** — the form *asks* what a season would be worth (`FALL_PRICE_BANDS`). `FALL_NO_HOLD_NOTE` is the one sentence that keeps the promise honest ("filling this out doesn't hold a spot"); it's reused verbatim by the page, the confirmation, and the broadcast so it can't drift. A dollar figure in either template is a test failure.
...
Fall 2026 "Coach Sam" clubs that **Enrichment Collective** runs in MCPS schools — Mon Olney / Tue Silver Spring / Wed Derwood / Thu Belmont (Olney). Partner-run like MVF: EC owns registration and payment, carries the general liability insurance, and collects the waivers and media releases, so **the NGA waiver gate does not apply** and no NGA Stripe path is involved. Sam is a 1099 contractor to EC.
...
- **Age floor exception.** NGA's 6–16 rule is unchanged and every NGA form still starts at 6. The Tuesday club is a K–2 school, so it carries `ageMin: 5` for EC's intro format — scoped to this file exactly as `MVF_AGE_MIN = 8` narrows in its own. Never propagate `ageMin: 5` to an NGA surface.
- **Dates are holds, not confirmed.** Stef has confirmed days/times/towns; the session calendar is still pending, so `dates` is a projected 9-week window (Sep 14 – Nov 12) with `status: "hold"`, unchecked against MCPS closures. Belmont's Thursday time is unannounced → `startTime: null` → all-day blocks. Replace `dates` and flip `status` when Stef confirms; the sync updates in place on the key.
...
- **Unpublished times ship all-day, never invented.** `MVF_PROGRAMS.timeLabel` is `null` until the MVF Fall Rec Guide publishes, so those classes emit `allDay: true` with `(time TBD)` in the title. Add the time to `mvf.ts` and the next sync converts them in place.
- **Fall 2026 items are `tentative: true`** — nothing is bookable, the permit isn't secured.
