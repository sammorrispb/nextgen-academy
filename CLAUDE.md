...
Pricing is **$20 per 1-hour slot, drop-in only — no subscription, no refunds**. Sessions split into Early and Late slots — pick one, or book **both for a discounted $35 two-hour bundle** (`STRIPE_DROPIN_BUNDLE_PRICE_ID`). A bundle is ONE Notion drop-in row (preserving the 1-row-per-checkout invariant the cancel/refund/idempotency paths rely on) that reserves a seat in both slots: the webhook increments both sessions' Registered count, cancel decrements both, and the row's `Second Slot Date`/`Second Slot Start Time` columns expand it into both slots' rosters (public + coach). The bundle is offered at checkout only when the adjacent slot is itself Open with seats — pairing logic in `src/lib/session-bundle.ts` (same day + venue, time-adjacent). Each session opens for registration **30 days ahead** and caps at 4 players per pickleball court.
...
- `STRIPE_DROPIN_BUNDLE_PRICE_ID` — price ID for the $35 two-hour bundle (both slots in one checkout). Optional — if unset, the bundle option still renders but checkout returns "Stripe price not configured"; keep it set in prod.
