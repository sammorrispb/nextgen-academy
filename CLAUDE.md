...
The CONFIRMED season announcement + one-click poll to **active families only** — distinct from the `/fall` demand-sizing survey above. Sam set the terms 2026-08-14 (`src/data/fall-poll-2026.ts`): **6 Sundays Sept 20 – Oct 25 at Earle B. Wood MS, Green 1:00–2:30 PM / Yellow 2:30–4:00 PM, 8 spots each, $225 per player** — so this template DOES quote the price (a real operator-set number; the no-quoting rule targets prices that don't exist yet).
...
Fall 2026 "Coach Sam" clubs that **Enrichment Collective** runs in MCPS schools — Mon Greenwood (Brookeville) / Tue Candlewood (Derwood) / Wed Rosemary Hills (Silver Spring) / Thu Belmont (Olney) / Fri Sherwood (Sandy Spring). Partner-run like MVF: EC owns registration and payment, carries the general liability insurance, and collects the waivers and media releases, so **the NGA waiver gate does not apply** and no NGA Stripe path is involved. Sam is a 1099 contractor to EC.
