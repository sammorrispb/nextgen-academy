...
- `withCronAlert(name, handler)` wrapping every cron; handler returns `{ ok, attempted, succeeded, failures[] }` so **partial** failures alert too.
...
- **No PII in alert bodies** (page IDs + counts only) — stays off the minor-PII egress surface.
