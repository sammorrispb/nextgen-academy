...
import {
  findMondayGirlsRegByPaymentIntent,
  findMondayGirlsRegByCheckoutSessionId,
  updateMondayGirlsRegStatus,
  type MondayGirlsRegistrationLookup,
} from "@/lib/notion-monday-girls-registrations";
import {
  mondayGirlsCancellationHtml,
  mondayGirlsCancellationText,
  mondayGirlsCancellationSubject,
} from "@/lib/email/monday-girls-cancellation";
import {
...
} from "@/lib/monday-girls-refund-policy";
import { findMondayGirlsSeasonGroup } from "@/data/monday-girls-season-2026";

/**
 * Monday Girls Beginner Group cancel + refund — structural mirror of cancel-fall.ts
 * (find row → refund per policy → flip Status → comms). The capacity guard in
 * /api/checkout-monday-girls counts rows with Status = "Confirmed", so flipping a
 * row to Refunded/Cancelled frees the seat by construction.
 *
 * Refund AMOUNT is never decided here — monday-girls-refund-policy.ts owns that.
 *
...
export type MondayGirlsCancelStatus = "Refunded" | "Cancelled";

export interface CancelMondayGirlsInput {
  paymentIntentId?: string;
  checkoutSessionId?: string;
  /**
   * Override the policy's refund decision. Omit to let
   * mondayGirlsRefundPolicyFor(todayET()) decide — that's the intended path.
   */
  refund?: MondayGirlsRefundPolicy;
  /**
...
export type CancelMondayGirlsResult =
  | {
      ok: true;
      pageId: string;
      status: MondayGirlsCancelStatus;
      refundedUsd: number;
      idempotent?: boolean;
      emailSent: boolean;
    }
...
async function alertAdmin(subject: string, body: string): Promise<boolean> {
  ...
  try {
    ...
    if (error) {
      console.error("[cancel-monday-girls] admin alert rejected", error);
      return false;
    }
    return true;
  } catch (err) {
    console.error("[cancel-monday-girls] admin alert threw", err);
    return false;
  }
}
...
async function issueRefund(
  ...
  try {
    ...
    console.error("[cancel-monday-girls] refund failed:", message);
...
async function sendCancellationEmail(
  ...
  const option = findMondayGirlsSeasonGroup(row.group);
  const input = {
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      bcc: ADMIN_EMAIL,
      replyTo: REPLY_TO,
      subject: mondayGirlsCancellationSubject(input),
      html: mondayGirlsCancellationHtml(input),
      text: mondayGirlsCancellationText(input),
    });
    if (error) {
      console.error("[cancel-monday-girls] cancellation email rejected", error);
      return false;
    }
    return true;
  } catch (err) {
    console.error("[cancel-monday-girls] cancellation email threw", err);
    return false;
  }
}

export async function cancelMondayGirlsRegistration(
  input: CancelMondayGirlsInput,
): Promise<CancelMondayGirlsResult> {
  ...
  const row = paymentIntentId
    ? await findMondayGirlsRegByPaymentIntent(paymentIntentId)
    : await findMondayGirlsRegByCheckoutSessionId(checkoutSessionId!);

  if (!row) {
    return {
      ok: false,
      reason: "not_found",
      message: "No Monday Girls roster row for that key",
    };
  }
  ...
  if (isTerminal(row.status)) {
    return {
      ok: true,
      pageId: row.pageId,
      status: row.status as MondayGirlsCancelStatus,
      refundedUsd: 0,
      idempotent: true,
      emailSent: false,
    };
  ...
  const today = todayET();
  const decision =
    input.refund ?? mondayGirlsRefundPolicyFor(today, { reason: input.reason });
  ...
  const status: MondayGirlsCancelStatus =
    refundedUsd > 0 || decision === "full" ? "Refunded" : "Cancelled";

  const updated = await updateMondayGirlsRegStatus(row.pageId, status);
  if (!updated) {
    return {
      ok: false,
...
export interface ObservedMondayGirlsRefund {
 ...
 */
export async function cancelMondayGirlsByPaymentIntent(
  paymentIntentId: string,
  refund: ObservedMondayGirlsRefund,
): Promise<CancelMondayGirlsResult> {
  // The money already moved (Stripe told us), so never try to refund again —
  // just reconcile the roster and tell the parent.
  const row = await findMondayGirlsRegByPaymentIntent(paymentIntentId);
  if (!row) {
    return {
      ok: false,
      reason: "not_found",
      message: "No Monday Girls roster row for that PI",
    };
  }
  ...
  const updated = await updateMondayGirlsRegStatus(row.pageId, "Refunded");
  if (!updated) {
    ...
    await alertAdmin(
      `[NGA] Monday Girls refund could not be reconciled to the roster`,
      message,
    );
