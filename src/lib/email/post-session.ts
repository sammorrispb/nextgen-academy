...
${
  commitUrl
    ...
    <div style="${s.actionCallout}">
      ...
      <p style="margin:6px 0 0 0;color:${c.text};font-size:14px;line-height:1.55;">
        Same time, same court, same crew &mdash; we&rsquo;ll auto-reserve ${escape(childFirst)}&rsquo;s spot for the next 4 weeks and only charge on weeks they actually play. Skip a week any time.
      </p>
      <p style="margin:14px 0 0 0;">
...
export function postSessionText(input: PostSessionInput): string {
  ...
  if (input.commitUrl) {
    lines.push(
      "",
      `Or lock in 4 weeks at once — same time, same court, same crew. We'll auto-reserve ${input.childFirst}'s spot and only charge on weeks they play. Skip any week.`,
