...
import { recordPlayoffScoreAction } from "./actions";
...
export default function PlayoffScoreForm({ group, slot, scoreA, scoreB, timed, played }: Props) {
  ...
  function save() {
    startTransition(async () => {
      setError(null);
      setMessage(null);
      const result = await recordPlayoffScoreAction({
        group,
        slot,
        scoreA: Number(a),
        scoreB: Number(b),
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <input type="number" inputMode="numeric" min={0} max={99} value={a} onChange={(e) => setA(e.target.value)} aria-label="Score, side A" className={input} />
      <span className="text-ngpa-white/45">–</span>
      <input type="number" inputMode="numeric" min={0} max={99} value={b} onChange={(e) => setB(e.target.value)} aria-label="Score, side B" className={input} />
      <label className="flex items-center gap-1.5 text-[11px] text-ngpa-white/65 select-none">
        <input type="checkbox" checked={onTime} onChange={(e) => setOnTime(e.target.checked)} className="accent-ngpa-teal" />
        ended on time
      </label>
      <button
        type="button"
